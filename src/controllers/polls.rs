...
pub async fn get_poll_by_id(pool: &SqlitePool, poll_id: i64) -> Result<PollWithCreator, sqlx::Error> {
    sqlx::query_as::<_, PollWithCreator>(
        "SELECT p.id, p.title, p.description, p.creator_id, u.username as creator_username, 
         p.created_at, p.expires_at
         FROM polls p
...
pub async fn get_poll_options(pool: &SqlitePool, poll_id: i64) -> Result<Vec<PollOption>, sqlx::Error> {
...
pub async fn create_poll(
    ...
    let expires_at = match chrono::DateTime::parse_from_rfc3339(&format!("{}:00Z", &form.expires_at)) {
        Ok(dt) => dt.with_timezone(&Utc),
        Err(_) => {
            error!("Invalid date format: {}", form.expires_at);
            return Err(sqlx::Error::ColumnDecode {
                index: "".to_string(),
                source: Box::new(std::io::Error::new(std::io::ErrorKind::InvalidData, "Invalid date format")),
            });
        }
    };
    ...
    let options = form.options.split(',')
        .map(|s| s.trim())
        .filter(|s| !s.is_empty())
        .collect::<Vec<&str>>();
    ...
    for option in options {
        ...
        sqlx::query(
            "INSERT INTO options (poll_id, text, is_date, date_time) VALUES (?, ?, ?, ?)"
        )
        .bind(poll_id)
        .bind(option)
        .bind(is_date)
...
pub async fn vote_on_poll(
    ...
    let existing_vote = sqlx::query(
        "SELECT id FROM votes WHERE user_id = ? AND option_id = ?"
    )
    .bind(user_id)
    .bind(option_id)
    .fetch_optional(pool)
